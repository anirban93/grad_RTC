    ...
    if( stringToSend && stringToSend != "") {
      //var selfID;
        for(var i = 0; i < maxCALLERS; i++ ) {
            ...
            //addToConversation(easyrtcid, "message", stringToSend); //app_data.name
...
function messageListener(easyrtcid, msgType, content) {
    for(var i = 0; i < maxCALLERS; i++) {
        if( easyrtc.getIthCaller(i) == easyrtcid) {
            ...
            addToConversation(easyrtcid, "message", content); //app_data.name
            //document.getElementById('textBlock').value = "";
        }
    }
}
...
function appInit() {
    ...
    easyrtc.setUsername(multi_data.user);
